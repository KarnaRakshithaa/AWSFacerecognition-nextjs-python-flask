...
credential = pd.read_csv("/Users/shashankkarna/Downloads/Face-recognition-using-Amazon-Rekognition-main/rekognitioninarvo_accessKeys copy.csv")
...
def get_face_name(face, image,COLLECTION_NAME):
    ...
    if face_exists(request):
        response = AWS_REKOG.search_faces_by_image(
            CollectionId=COLLECTION_NAME, Image=request, FaceMatchThreshold=65,QualityFilter='LOW',MaxFaces=10)
...
def face_recognition_saving_image(image,COLLECTION_NAME):
    ...
    for face in bounding_boxes:
        ...
        face_name = get_face_name(face, image, COLLECTION_NAME)
        end_time = time.time()
        faces_name.append(face_name)
        time_taken = end_time - start_time
        times.append(time_taken) 
        print(f"Time taken to recognize face: {time_taken:.2f} seconds")
